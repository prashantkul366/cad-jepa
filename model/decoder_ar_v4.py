...
class CADDecoderARV4(nn.Module):
    """
    8-block AR Transformer decoder for DeepCAD token sequences.
    V4: Dual conditioning — JEPA z* (8 tokens) + BERT text tokens (77 tokens).
    Memory = 85 tokens total. Cross-attention in blocks 2-7 attends to both.
    ...
    def __init__(self, latent_d: int = 512, d_model: int = 256,
        ...
        # ── BERT token projection: 768 → d_model ──────────────────────────
        # Projects all 77 BERT token embeddings into decoder dimension.
        # Combined memory = [8 JEPA tokens + 77 BERT tokens] = 85 tokens
    ...
    def forward(self, z_star: torch.Tensor,
        ...
        """
        z_star      : [B, latent_d]     JEPA encoder latent
        bert_hidden : [B, 77, 768]      BERT last hidden states (all tokens)
        ...
        # JEPA geometric memory: [B, 8, 256]
        mem_jepa = self.z_proj(z_star).view(B, self.n_mem, self.d_model)

        # BERT semantic memory: [B, 77, 256]
        mem_text = self.text_proj(bert_hidden)

        # Combined memory: [B, 85, 256]
        mem  = torch.cat([mem_jepa, mem_text], dim=1)
    ...
    def compute_loss(self, z_star: torch.Tensor,
        ...
        cmd_logits, args_logits = self.forward(z_star, bert_hidden, in_cmds, in_args)
    ...
    def generate(self, z_star: torch.Tensor,
        ...
        z_star       : [B, latent_d]
        bert_hidden  : [B, 77, 768]   BERT token embeddings
        ...
        for _ in range(max_len):
            ...
            cmd_logits, args_logits = self.forward(z_star, bert_hidden, seq_cmds, seq_args)
