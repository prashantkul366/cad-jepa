...
class MultiheadAttention(Module):
    ...
    # def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, rotary_emb=None):
        super(MultiheadAttention, self).__init__()
        self.embed_dim = embed_dim
        self.kdim = kdim if kdim is not None else embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
    ...
    Shape:
        ...
        #         key_padding_mask=key_padding_mask, need_weights=need_weights,
        #         attn_mask=attn_mask)
        # REPLACE from the first 'if not self._qkv_same_embed_dim' to end of method
        ...
        b_q, b_k, b_v = (self.in_proj_bias.chunk(3, dim=0)
                         if self.in_proj_bias is not None else (None, None, None))
        ...
        if attn_mask is not None:
            attn = attn + (attn_mask.unsqueeze(0) if attn_mask.dim() == 2 else attn_mask)
        ...
        out = out.permute(1, 0, 2).reshape(tgt_len, bsz, self.embed_dim)
        out = self.out_proj(out)

        if need_weights:
            return out, attn.view(bsz, self.num_heads, tgt_len, src_len).mean(dim=1)
        return out, None
